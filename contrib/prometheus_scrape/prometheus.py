...
def parse_extra_info(file_path: str) -> Dict[str, Dict[str, str]]:
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            if re.match(r'^pgmoneta_[a-zA-Z0-9_]+', line):
...
def find_best_example(metric_name: str, metrics_content: str) -> Optional[str]:
    ...
    for line in lines:
        if line.startswith(metric_name):
            ...
            # Prioritize lines with name="primary" (common server name)
            if 'name="primary"' in line:
                priority = 2
            elif '{' in line:
                priority = 1
            else:
...
def generate_manual_metric(metric: Dict, use_h2: bool = False) -> str:
    ...
    if attributes or values:
        if values and attributes:
            # 3-column table with both attributes and values
            ...
            # Only values, no attributes - use 2-column table for values
...
def main():
    parser = argparse.ArgumentParser(description='Generate pgmoneta Prometheus metrics documentation')
    parser.add_argument('port', type=int, help='Port to fetch metrics from (typically 7002)')
    ...
    # Fetch metrics
    try:
        response = requests.get(f'http://localhost:{args.port}/metrics', timeout=60)
    ...
    if args.manual:
        ...
        if md_output and args.md:
            generate_markdown_documentation(md_output, metrics_data, args.toc)
    
        if html_output and args.html:
            generate_html_documentation(html_output, metrics_data, args.toc)
    
    # Print summary
    print(f"Documentation generated:")
    if args.manual:
        print("- Manual: prometheus.md")
    else:
        if args.md:
...
def generate_manual_documentation(metrics_data: List[Dict]):
    """Generate manual documentation file."""
    with open('prometheus.md', 'w') as f:
        f.write("\\newpage\n\n")
        f.write("# Prometheus metrics\n\n")
        f.write("[**pgmoneta**][pgmoneta] has the following [Prometheus][prometheus] built-in metrics.\n\n")
        for metric in metrics_data:
            f.write(generate_manual_metric(metric, use_h2=False))

def generate_markdown_documentation(output_file: str, metrics_data: List[Dict], include_toc: bool):
    """Generate markdown documentation."""
    with open(output_file, 'w') as f:
        f.write("# pgmoneta Prometheus Metrics Documentation\n\n")
        f.write("This document contains all available metrics from the pgmoneta backup system.\n\n")
        ...
        if include_toc:
            ...
            f.write(f"\n**Total metrics: {len(metrics_data)}**\n\n")
            f.write("---\n\n")
...
def generate_html_documentation(output_file: str, metrics_data: List[Dict], include_toc: bool):
    """Generate HTML documentation."""
    with open(output_file, 'w') as f:
        # HTML header
        f.write("""<html>
  <head>
    <title>pgmoneta Prometheus Metrics Documentation</title>
    <style>
  ...
  <body>
  <h1>pgmoneta Prometheus Metrics Documentation</h1>
  <p>This document contains all available metrics from the pgmoneta backup system.</p>
""")
        ...
        if include_toc:
            ...
            f.write(f'<p>Total metrics: {len(metrics_data)}</p>\n')
            f.write('</div>\n')
